...
async def scostamenti_prezzi(
    ...
    if id_fornitore is not None:
        params.append(id_fornitore)
        filters.append(f"f.id = ${len(params)}")
    if solo_scostamenti:
...
async def esposizione_fornitori(
    ...
    if id_fornitore is not None:
        params.append(id_fornitore)
        filters.append(f"ft.id_fornitore = ${len(params)}")
    if urgenza:
...
async def dizionario(
    table_name: str | None = None,
    conn: asyncpg.Connection = Depends(get_conn),
):
    if table_name:
        rows = await conn.fetch(
            "SELECT * FROM _dizionario_dati WHERE table_name = $1 ORDER BY id", table_name
        )
    else:
        rows = await conn.fetch(
            "SELECT * FROM _dizionario_dati ORDER BY table_name, id"
        )
    return [dict(r) for r in rows]
